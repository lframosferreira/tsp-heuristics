...
def get_graph_from_tsplib95_file(
    filepath: str, is_euc_2d: bool = True
) -> networkx.Graph:
    if is_euc_2d:
        return tsplib95.load(filepath).get_graph()
    else:
        problem = tsplib95.load(filepath)
        problem._wfunc = lambda start, end: tsplib95.distances.pseudo_euclidean(
            problem.node_coords[start], problem.node_coords[end]
        )
...
def get_time_for_every_instance_and_dump(filename: str = "output.csv") -> None:
    ...
    for file in os.listdir(INSTANCES_FOLDER):
        graph: networkx.Graph = get_graph_from_tsplib95_file(
